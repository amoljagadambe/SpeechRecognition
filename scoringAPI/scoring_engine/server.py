...
import json
import datetime
from scoringAPI.scoring_engine import syllableScoring
from scoringAPI.scoring_engine import word_alignment
import random
...
word_scoring = api.namespace('/VoiceProcessor/word_scoring', description='Operations related to Recommendation')
...
sentence_scoring = api.namespace('train', description='Operations related to Recommendation')
	...
	def post(self):
		...
		if sentence == '':
			...
			# os.remove(upload_filename)
				...
				print(e)
				res = json.dumps({"result4": res + "& Details : {1}".format(e.errno, e.strerror)})
		else:
